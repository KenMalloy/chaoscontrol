...
- **Ownership:** Criticality Distillation lives on a dedicated `CriticalityDistillation(nn.Module)` (at e.g. `src/chaoscontrol/optim/criticality.py`), consumed by the runner next to ScOpt. **Not on `ScarcityAwareOptimizer`.** ScOpt produces pressure; CD consumes pressure + captured states + emits loss. This separation makes the `budget-only` falsifier (CD with no ScOpt pressure) trivially instantiable, and keeps ScOpt focused on parameter updates.
...
- **State capture** via a `capture_states()` context manager on `ChaosSSMCore`. Implemented by setting `self._capture_states_enabled = True` and writing `self._captured_states = states.detach()` in the forward. **Both diag call sites** in `src/chaoscontrol/core.py` must capture: the inlined fast-path branch inside `forward()` (line 573-583) AND the standalone `_forward_diag_scan` helper (line 411). Production `model.encode()` routes through `forward()`, not the helper, so missing that branch would capture nothing in practice. Regression test must exercise `ChaosStudentLM.encode()` or the outer block forward, not the private helper.
- **Trace bank storage:** the per-layer ring of `[D]` fp32 evidence tensors and per-step age ints are registered as `nn.Module` buffers on `CriticalityDistillation` (`self.register_buffer("bank_evidence", ...)` etc.). `nn.Module` does not auto-persist Python lists of tensors; buffers get `state_dict` serialization for free but must be explicitly registered. Stage 1b includes a state round-trip test (save, load, verify bank is restored).
- **Criticality loss** is composed in the runner's loss-accumulation block, alongside CE. `total_loss = ce + criticality_distill_weight * sum_layers(criticality_mse_l)`. Muon / AdamW / weight decay all see it as an extra gradient on `log_a` — no special-casing. **Non-seat channels must see exactly zero gradient from the criticality loss** (regression assertion: `log_a.grad[~seat_mask]` is all zero after the criticality backward).
- **Backend scope:** stage 1 wires capture for `a_mode == "diag"` only. `paired` and `full` have different A-matrix structure and don't map cleanly to "criticality = 1 - σ(log_a)" per channel — they need a redesigned target formulation, not just capture plumbing. Capture with `a_mode in {"paired", "full"}` raises a clear `NotImplementedError`.
- **Config surface** adds the keys listed in Defaults above, all with docstring and sane fallbacks so existing configs continue to work with the actuator disabled (`criticality_distill_weight = 0.0`).
