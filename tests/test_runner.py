...
class TestLoadConfig(unittest.TestCase):
    def test_load_config_from_yaml(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            ...
            path = f.name
        cfg = load_config(path, data_path="/tmp/data")
    ...
    def test_load_config_budget_override(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(cfg_dict, f)
            path = f.name
        cfg = load_config(path, data_path="/tmp/data", budget_seconds=999)
...
class TestBuildModel(unittest.TestCase):
    def test_build_model_ssm(self) -> None:
        cfg = ChaosControlConfig(data_path="/tmp", model_type="ssm", model_dim=16, num_layers=2)
